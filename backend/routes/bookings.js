...
router.get('/', protect, async (req, res) => {
    try {
        const bookings = await Booking.find().populate('slotId', 'date time');
        
        const now = moment().startOf('day');

        // Filter out past bookings and sort chronologically by slot's date and time
        const upcomingBookings = bookings.filter(booking => {
            if (!booking.slotId || !booking.slotId.date) return false;
            const startTimeString = booking.slotId.time.split(' - ')[0] || booking.slotId.time;
            const slotDateTime = moment(`${booking.slotId.date} ${startTimeString}`, 'DD MMM YYYY hh:mm A');
            return slotDateTime.isSameOrAfter(now);
        }).sort((a, b) => {
